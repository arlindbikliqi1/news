...
function getBreakingNews($con) {
    $breakingNews = [];
    if (isset($con) && $con) {
        $query = mysqli_query($con, "SELECT PostTitle, id FROM tblposts WHERE Is_Active = 1 ORDER BY PostingDate DESC LIMIT 5");
...
// Function to fetch categories for navigation
function getNavigationCategories($con) {
    $categories = [];
    if (isset($con) && $con) {
        $query = mysqli_query($con, "SELECT id, CategoryName FROM tblcategory WHERE Is_Active = 1 LIMIT 8");
        if ($query && mysqli_num_rows($query) > 0) {
            while ($row = mysqli_fetch_array($query)) {
                $categories[] = $row;
            }
...
// Fetch data
$breakingNewsItems = getBreakingNews($con);
$navCategories = getNavigationCategories($con);
...
<head>
    ...
    <style>
        ...
        /* Dropdown Styles - Using standard Bootstrap approach */
        .dropdown-menu {
            background: white;
...
<body>
            ...
            <div class="d-flex align-items-center">
                ...
                <div class="breaking-content">
                    <div class="news-ticker" id="newsTicker">
                        <?php if (!empty($breakingNewsItems)): ?>
                            <?php foreach ($breakingNewsItems as $index => $news): ?>
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav mx-auto">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                            <?php if (!empty($navCategories)): ?>
                                <?php foreach ($navCategories as $category): ?>
                                    <li>
                                        <a class="dropdown-item" href="category.php?catid=<?php echo htmlentities($category['id'])?>">
                    ...
                    <li class="nav-item">
                        <a class="nav-link <?php echo $current_page == 'about-us.php' ? 'active' : ''; ?>" href="about-us.php">
                            <i class="fas fa-info-circle me-1"></i>About
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link <?php echo $current_page == 'contact-us.php' ? 'active' : ''; ?>" href="contact-us.php">
                            <i class="fas fa-envelope me-1"></i>Contact
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link <?php echo $current_page == 'archive.php' ? 'active' : ''; ?>" href="archive.php">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Mobile menu close functionality - click outside to close
            document.addEventListener('click', function(event) {
                const navbar = document.querySelector('.navbar');
                const navbarCollapse = document.querySelector('.navbar-collapse');
                const navbarToggler = document.querySelector('.navbar-toggler');

                // Check if navbar collapse is open and click is outside navbar
                if (navbarCollapse && navbarCollapse.classList.contains('show')) {
                    if (!navbar.contains(event.target)) {
                        // Click is outside navbar, close it
            ...
            // **FIX:** Ensure navbar toggler explicitly toggles the collapse
            const navbarToggler = document.querySelector('.navbar-toggler');
            ...
            if (navbarToggler && navbarCollapse) {
                navbarToggler.addEventListener('click', function() {
                    ...
                    if (bsCollapse) {
                        ...
                        // If no instance exists, create one (shouldn't happen if Bootstrap JS is loaded correctly)
